...
interface ItemsTableProps {
  ...
  categoryUnits?: { [key: string]: string };
}
...
export function ItemsTable({
  ...
  return (
    <div className="w-full overflow-auto" onClick={isMobile ? handleTouchOutside : undefined}>
      {isMobile ? (
          ...
          {orderedCategories.map(category => (
            <div key={category} className="space-y-3">
              ...
              {groupedItems[category].map((item, index) => {
                const categoryUnit = getUnitForCategory(category);
                return (
                  <div 
                    key={item.id} 
                    className="relative"
