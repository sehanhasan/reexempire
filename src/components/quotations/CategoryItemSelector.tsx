        ...
        subcategories: [
          {
            ...
            pricingOptions: [
              { id: "PO-007", name: "Ceramic Tiles", price: 25, unit: "Sq.m" },
              { id: "PO-008", name: "Porcelain Tiles", price: 35, unit: "Sq.m" }
            ]
          }
        ] 
      },
  ...
  const filteredCategories = searchQuery
    ? categories.filter(cat => 
        ...
        cat.subcategories.some(sub => 
          ...
          sub.pricingOptions.some(option => 
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[800px] max-h-[80vh] flex flex-col overflow-hidden">
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1 overflow-hidden flex flex-col">
            ...
            {filteredCategories.map((category) => (
              <TabsContent 
                key={category.id} 
                value={category.id}
                className="flex-1 overflow-hidden mt-0 border rounded-md p-0"
              >
                                ...
                                <div key={option.id} className="flex items-center justify-between">
                                  <div className="flex items-center space-x-2">
                                    <Checkbox 
                                      id={itemKey}
                                      checked={isSelected}
                                      onCheckedChange={() => handleToggleItem(category, subcategory, option)}
                                    />
                                    <label
                                      htmlFor={itemKey}
        ...
        <div className="bg-muted/20 p-3 rounded-md mt-4">
          <div className="font-medium mb-2">Selected Items ({Object.keys(selectedItems).length})</div>
          {Object.keys(selectedItems).length > 0 ? (
            <ScrollArea className="max-h-[100px]">
              <ul className="space-y-1">
                {Object.values(selectedItems).map((item) => (
                  <li key={`${item.categoryId}-${item.subcategoryId}-${item.id}`} className="text-sm">
                    • {item.subcategoryName} - {item.description} (RM {item.price.toFixed(2)})
                  </li>
                ))}
              </ul>
            </ScrollArea>
