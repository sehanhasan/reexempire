...
import { Search, CalendarIcon, UserRound, ArrowLeft } from "lucide-react";
...
export default function EditWarrantyItem() {
  ...
  // Fetch warranty item
  const { data: warrantyItem, isLoading } = useQuery({
    queryKey: ['warranty-item', id],
    queryFn: async () => {
  ...
  useEffect(() => {
    if (warrantyItem) {
      ...
      form.setValue('invoice_id', warrantyItem.invoice_id || '');
      form.setValue('item_name', warrantyItem.item_name);
      form.setValue('serial_number', warrantyItem.serial_number || '');
      form.setValue('warranty_period_type', warrantyItem.warranty_period_type);
      form.setValue('issue_date', new Date(warrantyItem.issue_date));
      
      if (warrantyItem.warranty_period_type === 'custom') {
        form.setValue('end_date', new Date(warrantyItem.expiry_date));
      }
    }
  }, [warrantyItem, customers, form]);
  ...
  const updateWarrantyMutation = useMutation({
    ...
    onError: () => {
      toast({
        title: "Error",
        description: "Failed to update warranty item",
        variant: "destructive",
      });
    }
  });
    ...
    return (
      <div className={`${isMobile ? 'page-container' : 'mt-6'}`}>
        <div className="flex justify-center items-center h-64">
          <div className="text-muted-foreground">Loading warranty item...</div>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className={`${isMobile ? 'page-container' : 'mt-6'}`}>
      <PageHeader
        title="Edit Warranty Item"
        description="Update warranty information for this item"
      />
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                {form.watch('warranty_period_type') === 'custom' && (
                  <FormField
                    control={form.control}
                    name="end_date"
                    render={({ field }) => (
