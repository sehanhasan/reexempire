...
import { Edit, Trash, Loader2, Mail, Phone, MapPin } from "lucide-react";
...
export default function Customers() {
  ...
  const columns = [{
    ...
    }) => <div className="font-medium text-blue-600 cursor-pointer" onClick={() => handleView(row.original)}>
          {row.original.name}
        </div>
  }, {
    header: "WhatsApp",
    ...
    cell: ({
      ...
      row: {
        original: Customer;
          ...
          {row.original.phone ? <a href={`https://wa.me/${row.original.phone.replace(/^\+/, '')}`} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">
              {row.original.phone}
  ...
  return <div className="page-container">
      ...
      <div className="mt-2">
        <DataTable columns={columns} data={customers} searchKey="name" externalSearchTerm={searchTerm} onExternalSearchChange={setSearchTerm} />
      </div>
