...
export default function ViewInvoice() {
  ...
  const getStatusColor = status => {
    ...
    if (status === "Partial" || status === "Partially Paid") return "bg-blue-100 text-blue-800 hover:bg-blue-100";
    if (status === "Overdue") return "bg-red-100 text-red-600 hover:bg-red-100";
    if (status === "Unpaid") return "bg-amber-100 text-amber-800 hover:bg-amber-100";
                  ...
                  <tbody>
                    {categories.map((category, categoryIndex) => <React.Fragment key={category}>
                        ...
                        {groupedItems[category].map((item, idx) => <tr key={idx} className="border-b hover:bg-gray-50">
                             ...
                             <td className="text-right px-2 py-1 font-semibold text-gray-800">{formatAmount(!invoice.is_deposit_invoice && depositPaid > 0 ? item.quantity * item.unit_price : item.amount)}</td>
                           </tr>)}
                      </React.Fragment>)}
                  </tbody>
                </table>
