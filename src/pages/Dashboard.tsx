...
export default function Dashboard() {
  ...
  const renderActivityTab = () => <div className="space-y-6">
      <Card>
              ...
              {upcomingAppointments.map(appointment => <div key={appointment.id} className="flex items-center justify-between border-b pb-3 cursor-pointer hover:bg-gray-50 rounded-md p-2" onClick={() => showAppointmentDetails(appointment)}>
                  <div className="w-full">
                    <div className="flex justify-between items-start mb-1">
                      ...
                      <Badge className={appointment.status.toLowerCase() === "confirmed" || appointment.status.toLowerCase() === "scheduled" ? "bg-blue-500 text-white" : appointment.status.toLowerCase() === "completed" ? "bg-green-500 text-white" : appointment.status.toLowerCase() === "in progress" ? "bg-yellow-500 text-white" : appointment.status.toLowerCase() === "cancelled" ? "bg-red-500 text-white" : "bg-gray-500 text-white"}>
                        {appointment.status}
                      </Badge>
                    </div>
        ...
        <Card>
          ...
          <CardContent>
            {recentQuotations.length === 0 ? <p className="text-center py-10 text-muted-foreground">No quotations found</p> : <div className="space-y-4">
              ...
              return <div key={quotation.id} onClick={() => navigateToQuotation(quotation.id)} className="flex items-center justify-between pb-2 border-b cursor-pointer bg-gray-100 p-3 rounded-md">
                      <div>
                      ...
                      <div className="text-right">
                        ...
                        <Badge className={quotation.status === 'Approved' ? "bg-green-500 text-white" : quotation.status === 'Sent' ? "bg-blue-500 text-white" : quotation.status === 'Draft' ? "bg-gray-500 text-white" : "bg-green-500 text-white"}>
                          {quotation.status}
                        </Badge>
                      </div>
                    </div>;
        ...
        <Card>
          ...
          <CardContent>
            {recentInvoices.length === 0 ? <p className="text-center py-10 text-muted-foreground">No invoices found</p> : <div className="space-y-4">
              ...
              return <div key={invoice.id} className="flex items-center justify-between pb-2 border-b cursor-pointer hover:bg-gray-50 p-2 rounded-md" onClick={() => navigateToInvoice(invoice.id)}>
                      <div>
                        <h3 className="font-medium">
                          {customer.unit_number ? `#${customer.unit_number}` : "No Unit"}
                        </h3>
                      ...
                      <div className="text-right">
                        ...
                        <Badge className={invoice.payment_status === 'Paid' ? "bg-green-500 text-white" : invoice.payment_status === 'Unpaid' ? "bg-red-500 text-white" : "bg-yellow-500 text-white"}>
  ...
  const renderRevenueTab = () => <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent>
          <Chart chartData={revenueData} categories={["revenue"]} index="month" colors={["#3b82f6"]} valueFormatter={value => `RM ${value.toLocaleString()}`} height={isMobile ? 250 : 300} title="" />
        </CardContent>
      </Card>
    </div>;
  return <div className="page-container">
      ...
      {isMobile ? <div className="mt-0">
          <div className="flex border-b border-gray-200 bg-white rounded-t-lg">
            <button onClick={() => setActiveTab("activity")} className={`flex-1 py-3 px-1 text-center text-sm font-medium transition-colors duration-200 ${activeTab === "activity" ? "text-cyan-600 border-b-2 border-cyan-600" : "text-gray-500 hover:text-gray-700"}`}>
              Activity
            </button>
            <button onClick={() => setActiveTab("revenue")} className={`flex-1 py-3 px-1 text-center text-sm font-medium transition-colors duration-200 ${activeTab === "revenue" ? "text-cyan-600 border-b-2 border-cyan-600" : "text-gray-500 hover:text-gray-700"}`}>
              Revenue
            </button>
            <button onClick={() => setActiveTab("overview")} className={`flex-1 py-3 px-1 text-center text-sm font-medium transition-colors duration-200 ${activeTab === "overview" ? "text-cyan-600 border-b-2 border-cyan-600" : "text-gray-500 hover:text-gray-700"}`}>
              Overview
            </button>
          </div>
          ...
          <div className="mt-6">
            <div className="flex border-b border-gray-200 bg-white rounded-t-lg">
              <button onClick={() => setActiveTab("activity")} className={`py-4 px-6 text-base font-medium transition-colors duration-200 ${activeTab === "activity" ? "text-cyan-600 border-b-2 border-cyan-600" : "text-gray-500 hover:text-gray-700"}`}>
                Activity
              </button>
              <button onClick={() => setActiveTab("revenue")} className={`py-4 px-6 text-base font-medium transition-colors duration-200 ${activeTab === "revenue" ? "text-cyan-600 border-b-2 border-cyan-600" : "text-gray-500 hover:text-gray-700"}`}>
                Revenue
              </button>
              <button onClick={() => setActiveTab("overview")} className={`py-4 px-6 text-base font-medium transition-colors duration-200 ${activeTab === "overview" ? "text-cyan-600 border-b-2 border-cyan-600" : "text-gray-500 hover:text-gray-700"}`}>
                Overview
              </button>
            </div>
            
            <div className="mt-6">
              {activeTab === "activity" && <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <Card>
                    <CardHeader className="flex flex-row items-center justify-between">
                      <div>
                        <CardTitle className="text-lg text-cyan-600">Upcoming Appointments</CardTitle>
                  ...
                  <div className="space-y-4">
                    <Card>
                      ...
                      const customer = customersMap[quotation.customer_id] || {};
                      return <div key={quotation.id} className="flex items-center justify-between pb-2 border-b cursor-pointer hover:bg-gray-50 p-2 rounded-md" onClick={() => navigateToQuotation(quotation.id)}>
                                  <div>
                                    <h3 className="font-medium">
                                      {customer.unit_number ? `#${customer.unit_number}` : "No Unit"}
                                    </h3>
              ...
              {activeTab === "revenue" && renderRevenueTab()}
              {activeTab === "overview" && <div className="grid grid-cols-3 gap-4">
                  <StatCard title="Total Customers" value={stats.customers} trend={{
              value: 12,
              isPositive: true
            }} description="vs last month" icon={<Users className="h-4 w-4" />} onClick={() => navigate("/customers")} />
