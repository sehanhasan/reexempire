...
export default function Invoices() {
  ...
  useEffect(() => {
    ...
    if (searchTerm) {
      ...
      filtered = filtered.filter(invoice => invoice.reference_number?.toLowerCase().includes(search) || customers[invoice.customer_id]?.name?.toLowerCase().includes(search) || formatDate(invoice.issue_date)?.toLowerCase().includes(search));
    }
  ...
  const columns = [{
    ...
    cell: ({
      ...
      return <div className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${statusColor}`}>
            {status}
          </div>;
    }
  }, {
  ...
  return <div className="page-container">
      <PageHeader title="Invoices" actions={<Button onClick={() => navigate("/invoices/create")}>
        ...
        <CardContent>
          <div className="flex flex-col md:flex-row gap-3 mb-6">
            <div className="relative w-full md:w-80">
              ...
              <Input type="search" placeholder="Search invoices..." className="pl-8 h-10" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
            </div>
          ...
          <DataTable columns={columns} data={filteredInvoices} isLoading={loading} emptyMessage="No invoices found. Create your first invoice to get started." />
        </CardContent>
      </Card>

      <FloatingActionButton icon={<FilePlus className="h-5 w-5" />} onClick={() => navigate("/invoices/create")} />
