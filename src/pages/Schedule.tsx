...
export default function Schedule() {
  ...
  const renderWeekView = () => {
    ...
    return <div className="bg-white rounded-lg border shadow">
        <div className="grid grid-cols-8 border-b">
          <div className="py-4 px-2 text-center font-semibold text-muted-foreground border-r">
            Time
          </div>
          {weekDates.map((date, i) => <div key={i} className={`py-4 px-2 text-center ${date.toDateString() === new Date().toDateString() ? "bg-blue-50 font-semibold text-blue-600" : "font-semibold"}`}>
        ...
        <div className="grid grid-cols-8 divide-x">
          <div className="divide-y">
            {hours.map(hour => <div key={hour} className="h-24 p-1 text-xs text-muted-foreground">
          ...
          return <div key={dateIndex} className="divide-y relative">
                {hours.map(hour => <div key={hour} className="h-24 p-1 relative"></div>)}
              ...
              return <div key={eventIndex} className={`absolute rounded-md p-2 overflow-hidden shadow border-l-4 w-[calc(100%-8px)] cursor-pointer hover:opacity-90 ${event.status === "Confirmed" ? "bg-blue-50 border-blue-500" : event.status === "Pending" ? "bg-amber-50 border-amber-500" : event.status === "Completed" ? "bg-green-50 border-green-500" : "bg-gray-50 border-gray-500"}`} style={{
                ...
                left: '4px'
                      ...
                      {duration > 50 && <>
                          ...
                          {duration > 80 && <Badge className="mt-1 text-xs" variant="outline">
                              {event.status}
                            </Badge>}
                        </>}
                    </div>;
  ...
  return <div className="page-container">
      <PageHeader title="Schedule" actions={<Button className="flex items-center" onClick={() => navigate("/schedule/add")}>
      ...
      <div className="flex flex-wrap items-center justify-between mt-8 mb-4 gap-2">
        <div className="flex space-x-2">
          <Button variant="outline" size="sm" className={view === "week" ? "bg-blue-50 text-blue-700" : ""} onClick={() => setView("week")}>
            Week
          </Button>
          <Button variant="outline" size="sm" className={view === "month" ? "bg-blue-50 text-blue-700" : ""} onClick={() => setView("month")}>
            Month
          </Button>
        </div>
        
        <div className="flex items-center gap-2 md:gap-4 order-last md:order-none w-full md:w-auto justify-center my-2 md:my-0">
          <Button variant="outline" size="icon" onClick={view === "week" ? prevWeek : prevMonth}>
            <ChevronLeft className="h-4 w-4" />
          </Button>
          
          <h3 className="text-md md:text-lg font-semibold whitespace-nowrap">
            {view === "week" ? `${getWeekDates()[0].toLocaleDateString('en-US', {
            month: 'short',
            day: 'numeric'
          ...
          <Button variant="outline" size="icon" onClick={view === "week" ? nextWeek : nextMonth}>
      ...
      <div className="mt-2 overflow-x-auto pb-6">
        {isLoading ? <div className="bg-white rounded-lg border shadow p-8 text-center">
            Loading appointments...
          </div> : error ? <div className="bg-white rounded-lg border shadow p-8 text-center text-red-500">
            Error loading appointments. Please try again.
          </div> : view === "week" ? renderWeekView() : <div className="bg-white rounded-lg border shadow p-4">
      ...
      <AppointmentDetailsDialog open={isAppointmentDetailsOpen} onClose={() => setIsAppointmentDetailsOpen(false)} appointment={selectedAppointment} customer={selectedAppointment ? customersMap[selectedAppointment.customer_id] : null} assignedStaff={selectedAppointment?.staff_id ? staffMap[selectedAppointment.staff_id] : null} />
    </div>;
}