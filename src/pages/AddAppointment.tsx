...
export default function AddAppointment() {
  ...
  return <div className="page-container">
      <PageHeader title={isEditMode ? "Edit Appointment" : "Add Appointment"} description={isEditMode ? "Update an existing appointment or service visit." : "Schedule a new appointment or service visit."} actions={<Button variant="outline" onClick={() => navigate("/schedule")}>
      ...
      {isLoading ? <div className="flex items-center justify-center h-64">
        ...
        </div> : <form onSubmit={handleSubmit} className="mt-8 space-y-6">
          <Card>
            <CardHeader>
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex space-x-2">
                    <Button type="button" variant="outline" className="w-full justify-between" onClick={() => setIsCustomerSelectorOpen(true)}>
                      {selectedCustomer ? <span className="flex items-center">
                          <Check className="mr-2 h-4 w-4 text-green-500" />
                          {selectedCustomer.unit_number ? `#${selectedCustomer.unit_number} - ` : ""}
                          {selectedCustomer.name}
                        </span> : <span className="text-muted-foreground flex items-center">
                          <User className="mr-2 h-4 w-4" />
                          Select a customer
                        </span>}
                    </Button>
                  </div>
                  {selectedCustomer && <div className="text-sm text-muted-foreground">
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              {isLoadingStaff ? <div className="text-center py-4">Loading staff members...</div> : <div className="grid grid-cols-1 gap-4">
                  {staffMembers.map((staff: Staff) => <div key={staff.id} className="border rounded-md p-4">
                      <div className="flex items-center space-x-2">
                        <input type="checkbox" name="staff[]" value={staff.id} checked={selectedStaff.includes(staff.id)} onChange={() => toggleStaffSelection(staff.id)} className="h-4 w-4 accent-blue-600" id={`staff-${staff.id}`} />
                      ...
                      {selectedStaff.includes(staff.id) && <div className="mt-3 pl-6">
                          <Label htmlFor={`staff-notes-${staff.id}`} className="text-xs">
                            Notes for {staff.name}
                          </Label>
                          <Textarea id={`staff-notes-${staff.id}`} placeholder={`Instructions or notes for ${staff.name}...`} rows={2} className="mt-1 text-sm" value={staffNotes[staff.id] || ''} onChange={e => handleStaffNoteChange(staff.id, e.target.value)} />
                        </div>}
                    </div>)}
                </div>}
            </CardContent>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="space-y-2">
                ...
                <Textarea id="notes" placeholder="Enter any additional details about this appointment..." rows={4} value={notes} onChange={e => setNotes(e.target.value)} />
              </div>
      ...
      <CustomerSelector open={isCustomerSelectorOpen} onClose={() => setIsCustomerSelectorOpen(false)} onSelectCustomer={customer => {
      setSelectedCustomer(customer);
